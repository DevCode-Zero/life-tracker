...
export function DailyRoutineTimeline({ userId }: DailyRoutineTimelineProps) {
  ...
  const [editingRoutine, setEditingRoutine] = useState<DailyRoutine | null>(
    null,
  );
  ...
  const handleToggle = async (id: string, currentStatus: boolean) => {
    try {
      ...
      setRoutines((prev) =>
        prev.map((r) => (r.id === id ? { ...r, is_done: !currentStatus } : r)),
      );
    } catch (err: any) {
      toast.error(err.message);
    }
        ...
        <div className="relative space-y-0">
          {routines.map((item, i) => (
            <div key={item.id} className="group flex gap-3 pb-4 last:pb-0">
              ...
              >
                <div
                  className={`w-2.5 h-2.5 rounded-full mt-1 shrink-0 transition-colors ${item.is_done ? "bg-blue-500 shadow-[0_0_8px_rgba(59,130,246,0.5)]" : "bg-zinc-700"}`}
                />
                {i < routines.length - 1 && (
                  <div
                    className={`w-px flex-1 mt-1 ${item.is_done ? "bg-blue-500/40" : "bg-zinc-800"}`}
                  />
                )}
              </div>
              <div className="flex-1 flex items-center justify-between min-w-0">
                <div
                  ...
                  onClick={() => handleToggle(item.id, item.is_done)}
                >
                  <p className="text-[11px] text-zinc-500 font-mono leading-none mb-1">
                    {item.time}
                  </p>
                  <p
                    className={`text-sm transition-all ${item.is_done ? "text-zinc-500 line-through" : "text-white"}`}
                  >
                    {item.label}
                  </p>
                </div>
                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button
                    onClick={() => setEditingRoutine(item)}
