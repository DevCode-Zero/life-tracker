...
export async function getMonthlyBudgetSummary(
  ...
  const remaining_buffer =
    total_income - total_expenses - total_savings - total_invested;
  const savings_rate =
    total_income > 0
...
export async function updateGoalProgress(
  ...
  const { data: goal, error: queryError } = await supabase
    .from("financial_goals")
    .select("target_amount")
    .eq("id", goalId)
    .single();

  if (queryError || !goal) throw new Error("Goal not found");

  const is_completed = currentAmount >= goal.target_amount;
