...
export async function getHabitsWithLogs(
  ...
  const since = format(subDays(new Date(), Math.max(days, 30)), "yyyy-MM-dd");
  ...
  return (habits ?? []).map((habit) => {
    ...
    const last7 = habitLogs.filter(
      (l) => l.completed_at >= format(subDays(new Date(), 7), "yyyy-MM-dd"),
    );
    const last30 = habitLogs.filter(
      (l) => l.completed_at >= format(subDays(new Date(), 30), "yyyy-MM-dd"),
    );
